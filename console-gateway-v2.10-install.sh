#!/usr/bin/env bash
# Console Gateway v2.10 - Multi-port Exclusive Console Server (Tailscale + SSH + socat+flock)
# Fixes over v2.9 (third code review findings):
#   - Fix: apt-get uses DEBIAN_FRONTEND=noninteractive to prevent install hangs
#   - Fix: logrotate session log permissions changed to 0644 so support user can read
#   - Fix: addconsole uses udevadm settle instead of sleep 1 for reliable symlink wait
set -euo pipefail
...
✅ Console Gateway v2.10 installed (multi-port, exclusive, persistent naming)

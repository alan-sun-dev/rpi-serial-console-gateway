#!/usr/bin/env bash
# Console Gateway v2.7 - Multi-port Exclusive Console Server (Tailscale + SSH + socat+flock)
# Fixes over v2.6 (code review findings):
#   - Security: Tailscale install switched from curl|sh to official apt repo
#   - Fix: CONSOLE_BAUD_DEFAULT now validated at startup
#   - Fix: removed `local` misuse inside subshells
#   - Fix: removed dead code map_locked_append()
#   - Fix: next_available_port() now checks OS-level port conflicts via ss
#   - Fix: summary box truncates long alias names to prevent layout breakage
#   - Fix: removed unused jq dependency
#   - Fix: systemd template uses BindsTo= for proper device lifecycle
#   - Fix: addconsole step labels corrected (was [3/4] skipping migrate step)
#   - Fix: rmconsole awk now matches TAG+"console-gateway" for precise rule removal
# Previous v2.6 fixes retained:
#   - SSH AllowUsers auto-includes SUDO_USER
#   - addconsole udev rules use ENV{ID_USB_*} for multi-port adapter support
#   - console-detect shows USB interface number
# Previous v2.4 improvements retained:
...
ensure_packages() {
  ...
  apt-get install -y \
    ...
    ufw openssh-server \
    socat screen net-tools util-linux iproute2 procps unattended-upgrades
...
install_tailscale() {
  ...
  if ! command -v tailscale &>/dev/null; then
    ...
    curl -fsSL "https://pkgs.tailscale.com/stable/debian/${distro}.noarmor.gpg" \
      | tee /usr/share/keyrings/tailscale-archive-keyring.gpg >/dev/null
    curl -fsSL "https://pkgs.tailscale.com/stable/debian/${distro}.tailscale-keyring.list" \
...
EOF
  ...
  if [[ $i -eq 0 ]]; then
    ...
    log "✓ Map created with ${i} device(s): ${MAP_FILE}"
...
do_rmconsole() {
  ...
  # 2. Remove from map.tsv
  if [[ -f "\$MAP_FILE" ]]; then
    (
      flock -w 5 200 || { echo "Cannot acquire map lock"; exit 1; }
      tmp=\$(mktemp)
      grep -v "^\${symlink_name}\b" "\$MAP_FILE" > "\$tmp" || true
      cp "\$tmp" "\$MAP_FILE"
      rm -f "\$tmp"
    ) 200>"\$MAP_LOCK"
  ...
  # 3. Remove udev rule — match lines containing TAG+="console-gateway" and the symlink name
  if [[ -f "\$UDEV_RULES_FILE" ]]; then
    tmp=\$(mktemp)
    awk -v sym="\$symlink_name" '
      /^#/ { prev = \$0; next }
      index(\$0, "console-gateway") && index(\$0, sym) { prev = ""; next }
...
✅ Console Gateway v2.7 installed (multi-port, exclusive, persistent naming)

#!/usr/bin/env bash
# Console Gateway v2.8 - Multi-port Exclusive Console Server (Tailscale + SSH + socat+flock)
# Fixes over v2.7 (merged from two independent code reviews):
#   - Security: BAUD injection guard in bridge/session handler
#   - Security: systemd service sandboxing (NoNewPrivileges, ProtectSystem, etc.)
#   - Security: UFW reset changed from opt-out to opt-in (safe-by-default)
#   - Fix: rmconsole awk uses exact SYMLINK+="name" match (prevents substring collision)
#   - Fix: session handler trap cleans up owner file on exit
#   - Fix: Tailscale repo detection supports both Debian and Ubuntu
#   - Fix: ss port check uses strict pattern (prevents partial port match)
#   - Fix: generate_mapping() checks port conflicts at bootstrap
#   - Fix: rmconsole map.tsv uses awk exact field match instead of grep -v
#   - Fix: uninstall script removes itself
#   - Cleanup: removed unused net-tools dependency
set -euo pipefail
...
✅ Console Gateway v2.8 installed (multi-port, exclusive, persistent naming)

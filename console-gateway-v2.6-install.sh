#!/usr/bin/env bash
# Console Gateway v2.6 - Multi-port Exclusive Console Server (Tailscale + SSH + socat+flock)
# Fixes over v2.5:
#   - Fix: SSH AllowUsers now auto-includes the installer user (SUDO_USER) to
#     prevent locking out the admin after SSH hardening
#   - Fix: addconsole udev rules switched from ATTRS{} to ENV{ID_USB_*} properties
#     to correctly handle multi-port adapters (e.g. FTDI FT4232H) where
#     bInterfaceNumber and serial reside at different parent device levels;
#     ATTRS{} can only match attributes from ONE parent, causing silent rule failures
#   - Fix: addconsole now includes ENV{ID_USB_INTERFACE_NUM} in udev rules to
#     distinguish individual ports on multi-port adapters sharing the same S/N
#   - console-detect now shows USB interface number for multi-port adapters
# Previous v2.5 features retained:
#   - udev persistent naming: adapters get stable symlinks based on USB attributes
#   - Interactive 'consolectl addconsole' wizard
#   - map.tsv references stable symlink names (not ttyUSB0)
...
✅ Console Gateway v2.6 installed (multi-port, exclusive, persistent naming)

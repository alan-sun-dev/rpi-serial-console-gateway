#!/usr/bin/env bash
# Console Gateway v2.5 - Multi-port Exclusive Console Server (Tailscale + SSH + socat+flock)
...
for dev in /dev/ttyUSB* /dev/ttyACM*; do
  ...
  # Get USB attributes via udevadm
  ...
  serial=$(udevadm info -a -n "$dev" 2>/dev/null | grep -m1 'ATTRS{serial}' | sed 's/.*=="\(.*\)"/\1/')
  manufacturer=$(udevadm info -a -n "$dev" 2>/dev/null | grep -m1 'ATTRS{manufacturer}' | sed 's/.*=="\(.*\)"/\1/')
  product_name=$(udevadm info -a -n "$dev" 2>/dev/null | grep -m1 'ATTRS{product}' | sed 's/.*=="\(.*\)"/\1/')
...
next_available_port() {
  ...
  [[ \$maxp -eq \$PORT_BASE ]] && echo \$PORT_BASE || echo \$maxp
}
...
get_usb_attrs() {
  local dev_path="\$1"
  UDEV_VENDOR=\$(udevadm info -a -n "\$dev_path" 2>/dev/null | grep -m1 'ATTRS{idVendor}' | sed 's/.*=="\(.*\)"/\1/')
  UDEV_PRODUCT=\$(udevadm info -a -n "\$dev_path" 2>/dev/null | grep -m1 'ATTRS{idProduct}' | sed 's/.*=="\(.*\)"/\1/')
  UDEV_SERIAL=\$(udevadm info -a -n "\$dev_path" 2>/dev/null | grep -m1 'ATTRS{serial}' | sed 's/.*=="\(.*\)"/\1/')
  UDEV_MANUFACTURER=\$(udevadm info -a -n "\$dev_path" 2>/dev/null | grep -m1 'ATTRS{manufacturer}' | sed 's/.*=="\(.*\)"/\1/')
  UDEV_PRODUCT_NAME=\$(udevadm info -a -n "\$dev_path" 2>/dev/null | grep -m1 'ATTRS{product}' | sed 's/.*=="\(.*\)"/\1/')
}
...
do_addconsole() {
  ...
  printf "│  Device:    %-32s│\n" "\$selected_base"
  printf "│  Alias:     %-32s│\n" "\$alias_name"
  printf "│  Symlink:   /dev/%-27s│\n" "\$symlink_name"
  printf "│  Port:      %-32s│\n" "\$port"
  printf "│  Baud:      %-32s│\n" "\$baud"
  printf "│  Vendor:    %-32s│\n" "\${UDEV_VENDOR:-?}:\${UDEV_PRODUCT:-?}"
  printf "│  Serial:    %-32s│\n" "\${UDEV_SERIAL:-none}"
  echo "└─────────────────────────────────────────────┘"
  echo
  ...
  # ---- 1. Create udev rule ----
  echo
  echo "[1/4] Creating udev rule..."
  local rule='SUBSYSTEM=="tty"'
  rule="\${rule}, ATTRS{idVendor}==\"\${UDEV_VENDOR}\""
  rule="\${rule}, ATTRS{idProduct}==\"\${UDEV_PRODUCT}\""
  if [[ -n "\$UDEV_SERIAL" && "\$UDEV_SERIAL" != "none" ]]; then
    rule="\${rule}, ATTRS{serial}==\"\${UDEV_SERIAL}\""
  fi
  ...
  # ---- 3. Remove old ttyUSB entry if exists ----
...
do_rmconsole() {
  ...
  # 3. Remove udev rule
  if [[ -f "\$UDEV_RULES_FILE" ]]; then
    # Remove the rule line and its comment line above
    local tmp=\$(mktemp)
    awk -v sym="\$symlink_name" '
      /^#/ { prev = \$0; next }
      index(\$0, sym) { prev = ""; next }
...
✅ Console Gateway v2.5 installed (multi-port, exclusive, persistent naming)

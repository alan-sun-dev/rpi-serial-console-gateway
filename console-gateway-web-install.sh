...
#   - Security: CSRF token validation on all state-changing POST endpoints
#   - Security: WebSocket terminal validates port against map.tsv whitelist
#   - Security: removed redundant sudo from subprocess calls (app runs as root)
#   - Security: plaintext password no longer echoed at install end
#   - Bug fix: kick now actually kills active sessions (fuser) before restarting
#   - Bug fix: api_settings_port_update auto-creates drop-in if missing (was 404)
#   - Fix: eventlet.monkey_patch() added at top of app.py
#   - Fix: PASS_HASH generated by venv python after pip install (werkzeug available)
#   - Fix: backward compatible with legacy SHA-256 hash format (v1.0 upgrades)
#   - Fix: drop-in writes use native Python I/O instead of shell heredocs
set -euo pipefail
...
# FIX: monkey_patch must be the very first import so eventlet can cooperatively
# yield in all stdlib socket/time calls. Without this, socat reads block the
# event loop and terminal output stalls under load.
import eventlet
...
<body>
  <!-- FIX: CSRF token stored in a meta tag; JS reads it and sends as X-CSRF-Token header on all POST requests. -->
          ...
          <div class="col-md-4">
            <div class="card mb-3">
              ...
              <div class="card-body">
                ...
                <small class="text-muted d-block mt-1">Affects all ports not overridden by per-port drop-in</small>
  ...
  <script>
    // FIX: read CSRF token from meta tag once; include it in all POST requests.
    ...
    async function savePortSettings(device) {
      ...
      if (r && r.ok) {
        ...
        showToast('Error: ' + (r.error || 'save failed'), true);
      }
    }
    ...
    function showToast(message, isError = false) {
      ...
      const color = isError ? 'var(--brand-red)' : 'var(--brand-green)';
      el.innerHTML = `<div class="toast show text-light" style="background:var(--bg-card);border:1px solid var(--brand-primary)"><div class="toast-body"><i class="bi ${icon}" style="color:${color}"></i> ${escapeHtml(message)}</div></div>`;
...
${TS_IP:+  Tailscale: http://${TS_IP}:${CGW_PORT}}
  Login:     ${CGW_ADMIN_USER}
  Password:  (the value you supplied via --password, or 'consolegateway' if default)
  Install:   ${INSTALL_DIR}
  ...
  Change password:
    ...
    # Set: Environment=CGW_ADMIN_PASS_HASH=<paste hash here>
    sudo systemctl restart ${SERVICE_NAME}

  NOTE: This UI is HTTP only. Access via Tailscale VPN for security.
